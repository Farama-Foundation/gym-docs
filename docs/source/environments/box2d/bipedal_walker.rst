...
versions: - Normal, with slightly uneven terrain. - Hardcore, with
ladders, stumps, pitfalls.
...
A heuristic is provided for testing. It’s also useful to get
demonstrations to learn from. To run the heuristic:
...
Actions are motor speed values in the [-1, 1] range for each of the 4
...
To use to the *hardcore* environment, you need to specify the
...
-  v3: returns closest lidar trace instead of furthest; faster video
   recording
-  v2: Count energy spent
-  v1: Legs now report contact with ground; motors have higher torque
   and speed; ground has higher friction; lidar rendered less nervously.
-  v0: Initial version
